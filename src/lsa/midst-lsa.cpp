...
MidstLsa::update(const std::shared_ptr<Lsa>& lsa)
{
  ...
  for (const auto& name : namesToAdd) {
      ...
      uint64_t     rsn = mlsa->getNpl().getSeqNo();
 ...
 /* if (namesToAdd.size() != 0) {
    //m_mpl = &mlsa->getNpl();
    updated = true;
    return std::make_tuple(updated, namesToAdd, namesToAdd);*/
  }

  // Verify if the incoming MIDST prefix list removes any names advertised.
  ...
  // Verify that information has not been modified.
  for (const auto& name : newNames) {
    if ((mlsa->getNpl().getDistance(name) != m_mpl.getDistance(name)) ||
        (mlsa->getNpl().getAnchor(name) != m_mpl.getAnchor(name)) ||
        (mlsa->getNpl().getSeqNo(name) != m_mpl.getSeqNo(name))) {
      //m_mpl = mlsa->getNpl();
      updated = true;
      return std::make_tuple(updated, namesToAdd, namesToAdd);
    }
  ...
  // If we reach this line, nothing has changed and updated is FALSE.
