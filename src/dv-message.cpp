...
const std::string DvMessage::REGULAR_INTEREST_COMPONENT = "RE";
const std::string DvMessage::UPDATE_TABLE_INTEREST_COMPONENT = "UT";
...
ndn::Name
DvMessage::buildMidstInterestPrefix(ndn::Name neighbor, InterestType intType) const
{
  ndn::Name midstInterest = neighbor;
  midstInterest.append(NLSR_COMPONENT);
  midstInterest.append(DIST_VECTOR_COMPONENT);

  if (intType == UPDATE_TABLE_INT) {
    midstInterest.append(UPDATE_TABLE_INTEREST_COMPONENT);
  }
  else {
    midstInterest.append(REGULAR_INTEREST_COMPONENT);
  }
...
{
  ndn::Name interestName = buildMidstInterestPrefix(neighbor, REGULAR_INT);
...
DvMessage::processInterest(const ndn::Name& name, const ndn::Interest& interest)
{
  ...
  std::string intType = interestName.get(-3).toUri();
  ...
  if (m_confParam.getAdjacencyList().isNeighbor(neighbor)) {
    ...
    if (intType == UPDATE_TABLE_INTEREST_COMPONENT) {
      NLSR_LOG_DEBUG("Processing Update Table Interest: " << interestName);
      ...
      NLSR_LOG_DEBUG("Processing Regular Interest: " << interestName);
...
DvMessage::onContentValidated(const ndn::Data& data)
{
  ...
  if (lsaPosition >= 0) {
    ...
    if (m_lsdb.wireDecode(dataBlock)) {
    ...
    if (lsaPtr->getOriginRouter() == m_confParam.getRouterPrefix()) {
      ...
      for (const auto& adj : lsaPtr->getAdl().getAdjList()) {
        ...
        if ((adj.getName() != origNeighbor) &&
          ...
          expressUtInterest(adj.getName(), m_confParam.getInterestResendTime());
        }
      }
    }
  }
...
void
DvMessage::expressUtInterest(const ndn::Name& neighbor, uint32_t seconds)
{
  ndn::Name interestName = buildMidstInterestPrefix(neighbor, UPDATE_TABLE_INT);
  ...
  m_face.expressInterest(interest,
                   std::bind(&DvMessage::onContentUtData, this, _1, _2),
                   [this] (const ndn::Interest& interest, const ndn::lp::Nack& nack)
                   {
                     NDN_LOG_TRACE("Received Nack with reason " << nack.getReason());
                     NDN_LOG_TRACE("Treating as timeout");
...
void
DvMessage::onContentUtData(const ndn::Interest& interest, const ndn::Data& data)
{
  NLSR_LOG_DEBUG("Received UT Data Acknowledgement: " << data.getName());
}
